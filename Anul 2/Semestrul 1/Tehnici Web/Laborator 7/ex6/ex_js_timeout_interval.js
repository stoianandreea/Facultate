...
function sirOra(){
	d = new Date();    // mereu la new Date pune data curenta
	hh = format2Cifre(d.getHours());
	mm = format2Cifre(d.getMinutes());
	ss = format2Cifre(d.getSeconds());
	ms = d.getMilliseconds();
	return `${hh}:${mm}:${ss} ${ms}`;   // face un string fara sa folosim +
...
window.onload = function(){
	...
	butonInterval.onclick = function(){
		if(idInterval == -1)
			idInterval = setInterval(afisInDiv, 1000, this.nextElementSibling.nextElementSibling); //selecteaza div-ul de dupa cele 2 butoane
	}
