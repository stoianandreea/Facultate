...
function creeazaRand(tipCelula, vector){
	var tr = document.createElement("tr"); // sa se creeze un rand
	for(x of vector){
		var celula = document.createElement(tipCelula);
		celula.innerHTML = x;      // continutul celulei trebuie sa fie valoarea din vector
		tr.appendChild(celula);    // adaugam celula in rand
	}
	return tr;
}
...
function creeazaTabel(elevi){
	...
	tabel.id = "tab";
	var thead = document.createElement("thead"); // creare thead
	tabel.appendChild(thead); // adugare thead in tabel (aici e variabila, nu tipul de date)
	...
	for(elev of elevi){ // variabila elev sa aiba pe rand ca valoare fiecare element din elevi
		rand = creeazaRand("td", Object.values(elev));
		console.log("Valori:"); 
		console.log(Object.values(elev));
		...
		rand.onclick=function(e){
			if(e.ctrlKey){
				this.remove(); //nu rand.remove() pt ca mereu va sterge ultimul rand
...
window.onload = function() {
	...
	document.getElementById("add_inceput").onclick = function() {
		...
		//getElementsByTagName=colectia de taguri tbody din tabel si din ea luam primul element (si unicul in cazul nostru), cel de pe pozitia [0]
...
window.onkeypress = function(e){
	...
	var tasta = String.fromCharCode(cod).toUpperCase(); //conversie din ASCII
	// "a".charCodeAt(0) conversie in ASCII
...
function printLog(logtext) {
	...
	//15
	info.scrollTop = info.scroll.Height;
}