...
class Culoare{
	constructor(_r = 0, _g = 0, _b = 0) {     // definiti trei parametri _r, _g, _b cu valori implicite 0
		this.r = _r;
		this.g = _g;
		this.b = _b;
	}
	...
	invert(){
		return new Culoare(255-this.r, 255-this.g, 255-this.b);   // calculati culoarea complementara
	}
};
...
function ransom(sir){
	...
	for (i = 0; i < sir.length; i++){    // parcurgere sir
		var patt1 = /\s/g;
		...
		if(!sir[i].match(patt1)){ // daca am spatiu
			...
			nr = randInt(0,2);
			lit.style.fontStyle = (nr%2==0?"italic":"normal"); // setare random  daca sa fie font italic sau nu (ca si la bold)
			fonturi = ["Times New Roman","Comic Sans MS","Impact","Arial Black","Courier New","Lucida Console","Trebuchet MS"];
			lit.style.fontFamily = fonturi[randInt(0, fonturi.length)]; // element aleator din fonturi
